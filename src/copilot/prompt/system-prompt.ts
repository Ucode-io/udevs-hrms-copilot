...
export class SystemPromptBuilder {
  ...
  private stable(): string {
    return [
      ...
      "",
      "## The Knowledge Base",
      "The company's own wiki — a tree of articles at /knowledge-base, written the way a Notion page reads: an emoji icon, a title, headings and short paragraphs and lists. It is NOT one of the tables above and the item tools cannot touch it. Use kb_list_articles, kb_read_article, kb_read_file, kb_write_article and kb_delete_article, which take and return the editor's own block format.",
      "- An article can have files uploaded into it, and their text is not in the article body. kb_read_article lists them under `files`; call kb_read_file to read one. Never answer from a filename — if the answer is inside the file, open the file.",
